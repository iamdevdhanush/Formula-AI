import { getCircuits } from '../services/circuits.service.js';
...
export function createCircuitsPage(router, signal) {
  ...
  hero.innerHTML = '<div style="position:absolute;top:-60px;right:-40px;width:280px;height:280px;background:var(--accent-cyan);filter:blur(100px);opacity:0.06;border-radius:50%"></div><div style="position:relative;z-index:1"><div style="font-size:11px;font-weight:700;color:var(--accent-cyan);text-transform:uppercase;letter-spacing:0.12em;margin-bottom:10px">Current Calendar</div><h1 style="font-size:clamp(28px,4vw,48px);font-weight:900;letter-spacing:-0.04em;color:var(--text-primary)">Circuits</h1><p style="font-size:var(--text-base);color:var(--text-muted);margin-top:8px">Circuits on the current Formula 1 World Championship calendar. Data sourced from OpenF1 API.</p></div>';
...
async function loadCircuits() {
  try {
    ...
    if (circuits.length === 0) {
      grid.appendChild(createEmptyState('Circuit data is currently unavailable.'));
      return;
    }
    ...
    if (grid) {
      grid.innerHTML = '';
      grid.appendChild(createErrorState(e.message, loadCircuits));
    }
  }
}
